...
class DispatchView(MethodView):
    ...
    def update_dispatch(self, dispatch_id):  # noqa: E501
        ...
        try:
            if request.content_type.startswith("multipart/form-data"):
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                files = request.files.getlist("images")
                self.dispatch_use_case.update_dispatch(dispatch_data, dispatch_id, files, internal_process)
                response["error_code"] = 0
    ...
    def get_history_dispatch(self):
        internal_process = (None, None)
        function_name = "get_history_dispatch"
        response = {}
        status_code = 500
        try:
            if connexion.request.headers:
                ...
                message = f"start request: {function_name}, channel: {request.headers.get('channel')}"
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                results = self.dispatch_use_case.get_history_dispatch(request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
    ...
    def get_resume_graphs(self):
        ...
        try:
            if connexion.request.headers:
                ...
                logger.info(message, internal=internal_transaction_id, external=external_transaction_id)
                headers = {k.lower(): v for k, v in request.headers.items()}
                result = self.dispatch_use_case.get_resume_graphs(headers, request.args, internal_transaction_id, external_transaction_id)
                response["error_code"] = 0
